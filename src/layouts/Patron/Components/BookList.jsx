...
function BookList(props) {
    ...
    const gg = (mid, mtitle, mcopies) => {
        return (
            <Button style={{display:"inline-block",margin:0,padding:"0 30px 0 30px",maxWidth:'100%'}} onClick={() => getInfo(mid, mtitle)} disabled={disableWhenZero(mcopies)}>BORROW</Button>
        )
    }
            ...
            await getDocs(colRefMaterial).then( (qSnapshot)=>{
                ...
                let materials =
                    qSnapshot.docs.map((docMaterial)=>({
                        ...
                        m_dept      : docMaterial.data().m_dept,
                        m_pub_date  : docMaterial.data().m_pub_date,
                        m_btn       : gg(docMaterial.id,docMaterial.data().m_title,docMaterial.data().m_copies)
                    }))
                    console.log('MATERIALS\t', materials)
                setMaterialResult(materials)
    ...
    const [columns] = useState([
        ...
        { name: 'm_pub_date', title: 'PUB YEAR' },
        { name: 'm_btn', title: 'BORROW' },
    ]);
